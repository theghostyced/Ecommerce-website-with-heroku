...
class ControllerExtensionModuleSoOnepageCheckout extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$params_module = array('name'=>$params_general['so_onepagecheckout_name']);
			
			if (!isset($this->request->get['module_id'])) {
				$this->model_extension_module->addModule('so_onepagecheckout', $params_module);
			...
			if($action == "save_edit") {
				$this->response->redirect($this->url->link('extension/module/so_onepagecheckout', 'token=' . $this->session->data['token'] . '&module_id='.$module_id, 'SSL'));
			}elseif($action == "save_new"){
				$this->response->redirect($this->url->link('extension/module/so_onepagecheckout', 'token=' . $this->session->data['token'], 'SSL'));
			}else{
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/module/so_onepagecheckout', 'token=' . $this->session->data['token'], 'SSL')
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/so_onepagecheckout', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);			
		}
		...
		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/so_onepagecheckout', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('extension/module/so_onepagecheckout', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
	...
	function install() {
		...
		$data_module = array('name'=>'So Onepage Checkout');
		$this->model_setting_setting->editSetting('so_onepagecheckout', $data);
		$this->model_extension_module->addModule('so_onepagecheckout', $data_module);
	}
	...
	function uninstall() {
		...
		$this->model_extension_module->deleteModulesByCode('so_onepagecheckout');
	}
}