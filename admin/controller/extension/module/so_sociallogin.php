...
class ControllerExtensionModuleSosocialLogin extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$params_module = array_merge($params, array('name'=>$params['so_sociallogin_name']));
		...
		if (isset($params['so_sociallogin_status'])) {
			$data['status'] = $params['so_sociallogin_status'];
		} elseif (!empty($module_info)) {
			$data['status'] = isset($module_info['so_sociallogin_status']) ? $module_info['so_sociallogin_status'] : '';
		} else {
			$data['status'] = 0;
		}
	...
	function install() {
		...
		$data_module = array_merge($data, array('name'=>$data['so_sociallogin_name']));
